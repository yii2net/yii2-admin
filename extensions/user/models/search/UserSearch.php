...
namespace openadm\admin\extensions\user\models\search;
...
class UserSearch extends User
{
    ...
    public function search($params)
    {
        ...
        if(isset($params['UserSearch']['banned_at'])){
            $banned_at = is_numeric($params['UserSearch']['banned_at']) ? intval($params['UserSearch']['banned_at']) : null;
            $cond = null;
            if($banned_at === 1){
                //正常
                $cond = ['is','banned_at',new \yii\db\Expression('NULL')];
            }elseif($banned_at === 0){
                //禁用
                $cond = ['is', 'banned_at' , new \yii\db\Expression('NOT NULL')];
            }else{
                //全部
                unset($params['UserSearch']['banned_at']);
            }
            if($cond){
